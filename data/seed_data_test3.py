"""
Test Case 3: OOO for 14 days from February 1-14, 2024
This script creates mock data for a 14-day OOO period with Valentine's Day context,
Q1 planning, security issues, and system maintenance scenarios.
"""

import sqlite3
import os

os.makedirs("data/databases", exist_ok=True)

def create_email_database():
    """Create and populate email database for Test Case 3"""
    conn = sqlite3.connect("data/databases/emails.db")
    cursor = conn.cursor()
    
    # Create emails table if it doesn't exist
    cursor.execute("""
        CREATE TABLE IF NOT EXISTS emails (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            custom_id TEXT UNIQUE,
            sender TEXT NOT NULL,
            subject TEXT NOT NULL,
            body TEXT NOT NULL,
            received_date TEXT NOT NULL,
            is_read BOOLEAN DEFAULT 0,
            thread_id TEXT,
            meeting_date TEXT,
            meeting_duration INTEGER,
            attendees TEXT
        )
    """)
    
    # Important emails (20% of total - 16 out of 80)
    important_emails = [
        # Critical security and system issues
        ("email_021", "security@company.com", "URGENT: Critical Security Vulnerability Detected", "Critical security vulnerability in authentication system. Patch deployment required immediately to prevent data breach.", "2024-02-05 09:00:00", 0, "thread_sec_021", "2024-02-05 14:00:00", 120, "security@company.com,cto@company.com,devops@company.com"),
        ("email_026", "legal@company.com", "URGENT: Major Client Contract Renewal - Feb 10th Deadline", "Major client contract renewal deadline approaching on Feb 10th. Legal review and approval required immediately.", "2024-02-08 10:30:00", 0, "thread_contract_026", "2024-02-10 09:00:00", 90, "legal@company.com,ceo@company.com,sales@company.com"),
        ("email_028", "security@company.com", "Security Audit Preparation - Feb 11th", "Annual security audit scheduled for Feb 11th. Your system access and documentation review required.", "2024-02-09 11:00:00", 0, "thread_audit_028", "2024-02-11 10:00:00", 180, "security@company.com,compliance@company.com,it@company.com"),
        ("email_030", "support@company.com", "CRITICAL: Major Customer System Outage", "Major customer experiencing system outage. Your technical expertise needed for resolution by Feb 12th.", "2024-02-11 08:00:00", 0, "thread_outage_030", None, None, "support@company.com,devops@company.com,cto@company.com"),
        ("email_031", "sales@company.com", "Enterprise Deal Closing - Feb 14th", "Major enterprise deal closing requires your technical consultation and approval on Feb 14th.", "2024-02-12 10:00:00", 0, "thread_deal_031", "2024-02-14 11:00:00", 90, "sales@company.com,ceo@company.com,legal@company.com"),
        ("email_032", "devops@company.com", "Infrastructure Migration - Feb 13th", "Critical infrastructure migration requires your oversight and technical guidance on Feb 13th.", "2024-02-13 08:30:00", 0, "thread_migration_032", "2024-02-13 20:00:00", 360, "devops@company.com,operations@company.com,dev-team@company.com"),
        
        # Q1 Planning and Budget
        ("email_027", "finance@company.com", "Q1 Budget Approval - Feb 9th", "Q1 budget requires your approval before implementation. Critical for business operations.", "2024-02-08 14:00:00", 0, "thread_budget_027", "2024-02-09 10:00:00", 60, "finance@company.com,ceo@company.com,managers@company.com"),
        ("email_033", "product@company.com", "Q1 Product Roadmap Review", "Q1 product roadmap requires your review and approval. Strategic planning session on Feb 7th.", "2024-02-06 15:00:00", 0, "thread_roadmap_033", "2024-02-07 14:00:00", 120, "product@company.com,architects@company.com,managers@company.com"),
        
        # Performance and Compliance
        ("email_034", "compliance@company.com", "Quarterly Compliance Review Due", "Quarterly compliance review is due Feb 15th. Ensure all documentation is updated.", "2024-02-12 09:00:00", 0, "thread_compliance_034", None, None, "compliance@company.com,legal@company.com,all@company.com"),
        ("email_035", "hr@company.com", "Annual Performance Review Deadline", "Annual performance review deadline is Feb 14th. Submit your self-assessment.", "2024-02-13 10:00:00", 0, "thread_performance_035", None, None, "hr@company.com,managers@company.com"),
        
        # System Maintenance
        ("email_036", "operations@company.com", "Production System Maintenance Window", "Production system maintenance scheduled for Feb 6th. Your approval required for downtime.", "2024-02-05 16:00:00", 0, "thread_maintenance_036", "2024-02-06 02:00:00", 240, "operations@company.com,devops@company.com,cto@company.com"),
        ("email_037", "database@company.com", "Database Performance Optimization", "Database performance optimization project requires your technical review on Feb 8th.", "2024-02-07 11:00:00", 0, "thread_db_037", "2024-02-08 15:00:00", 90, "database@company.com,devops@company.com,architects@company.com"),
        
        # Valentine's Day Context
        ("email_038", "events@company.com", "Valentine's Day Team Building Event", "Valentine's Day team building event on Feb 14th. Your participation is encouraged.", "2024-02-12 14:00:00", 0, "thread_valentine_038", "2024-02-14 16:00:00", 120, "events@company.com,hr@company.com,all@company.com"),
        ("email_039", "marketing@company.com", "Valentine's Day Campaign Launch", "Valentine's Day marketing campaign launch requires your approval on Feb 13th.", "2024-02-12 13:00:00", 0, "thread_campaign_039", "2024-02-13 11:00:00", 60, "marketing@company.com,product@company.com,managers@company.com"),
        
        # Additional Critical Items
        ("email_040", "qa@company.com", "Release Testing - Feb 14th", "Final release testing requires your approval before production deployment on Feb 14th.", "2024-02-14 09:00:00", 0, "thread_testing_040", "2024-02-14 10:00:00", 120, "qa@company.com,dev-team@company.com,product@company.com"),
        ("email_041", "security@company.com", "Security Incident Response - Feb 14th", "Security incident requires immediate response and your technical expertise on Feb 14th.", "2024-02-14 12:00:00", 0, "thread_incident_041", "2024-02-14 13:00:00", 180, "security@company.com,cto@company.com,operations@company.com"),
    ]
    
    # Noise emails (80% of total - 64 out of 80)
    noise_emails = [
        # Training and Development
        ("email_042", "training@company.com", "New Employee Onboarding", "Welcome our new team members! Onboarding sessions are scheduled throughout the week.", "2024-02-04 09:30:00", 0, "thread_onboarding_042", None, None, None),
        ("email_043", "research@company.com", "Market Research Findings", "Latest market research findings are now available in the shared drive. Some interesting trends!", "2024-02-04 13:00:00", 0, "thread_research_042", None, None, None),
        ("email_044", "compliance@company.com", "Compliance Review Reminder", "Quarterly compliance review is due. Please ensure all documentation is up to date.", "2024-02-05 10:30:00", 0, "thread_compliance_042", None, None, None),
        ("email_045", "sales@company.com", "Q1 Sales Targets Update", "Q1 sales targets have been updated based on current market conditions. Let's crush these goals! 💪", "2024-02-05 12:45:00", 0, "thread_sales_042", None, None, None),
        ("email_046", "product@company.com", "Feature Request Summary", "Feature request summary from customer feedback sessions is now available. Great insights!", "2024-02-06 15:15:00", 0, "thread_features_042", None, None, None),
        
        # Office and Administrative
        ("email_047", "admin@company.com", "Office Supplies Update", "Office supplies have been ordered and will arrive next week. Thanks for your patience!", "2024-02-06 11:00:00", 0, "thread_supplies_042", None, None, None),
        ("email_048", "it@company.com", "Software Update Reminder", "Reminder: Please update your software to the latest version for security patches.", "2024-02-07 10:00:00", 0, "thread_update_042", None, None, None),
        ("email_049", "finance@company.com", "Expense Report Reminder", "Monthly expense reports are due. Please submit your February expenses by month-end.", "2024-02-07 14:00:00", 0, "thread_expenses_042", None, None, None),
        ("email_050", "hr@company.com", "Team Restructuring", "Important team restructuring decisions require your input and approval.", "2024-02-08 15:00:00", 0, "thread_restructure_042", None, None, None),
        
        # Social and Events
        ("email_051", "events@company.com", "Valentine's Day Office Decorations", "Help us decorate the office for Valentine's Day! Sign up for the decoration committee.", "2024-02-09 12:00:00", 0, "thread_decorations_042", None, None, None),
        ("email_052", "social@company.com", "Valentine's Day Gift Exchange", "Valentine's Day gift exchange sign-up is now open. Maximum $10 per gift.", "2024-02-10 10:00:00", 0, "thread_gifts_042", None, None, None),
        ("email_053", "catering@company.com", "Valentine's Day Lunch Menu", "Special Valentine's Day lunch menu available. Pre-order by Feb 12th.", "2024-02-11 09:00:00", 0, "thread_lunch_042", None, None, None),
        
        # More noise emails to reach 64 total
        ("email_054", "newsletter@company.com", "Weekly Company Newsletter", "Check out our latest company newsletter with exciting updates!", "2024-02-01 08:00:00", 0, "thread_news_042", None, None, None),
        ("email_055", "feedback@company.com", "Customer Satisfaction Survey", "Monthly customer satisfaction survey results are in. Great job team!", "2024-02-01 11:00:00", 0, "thread_survey_042", None, None, None),
        ("email_056", "recruitment@company.com", "New Job Postings", "New job postings are available. Refer a friend and earn a bonus!", "2024-02-02 09:00:00", 0, "thread_jobs_042", None, None, None),
        ("email_057", "wellness@company.com", "Employee Wellness Program", "Join our employee wellness program. Free gym membership included!", "2024-02-02 14:00:00", 0, "thread_wellness_042", None, None, None),
        ("email_058", "parking@company.com", "Parking Permit Renewal", "Parking permits expire at month-end. Renew online to avoid fines.", "2024-02-03 10:00:00", 0, "thread_parking_042", None, None, None),
        ("email_059", "cleaning@company.com", "Office Cleaning Schedule", "Updated office cleaning schedule. Deep clean scheduled for Feb 15th.", "2024-02-03 15:00:00", 0, "thread_cleaning_042", None, None, None),
        ("email_060", "utilities@company.com", "Utility Bill Update", "Monthly utility bills are available for review. Contact admin for questions.", "2024-02-04 08:30:00", 0, "thread_utilities_042", None, None, None),
        ("email_061", "insurance@company.com", "Health Insurance Update", "Health insurance enrollment period is open. Review your coverage options.", "2024-02-04 16:00:00", 0, "thread_insurance_042", None, None, None),
        ("email_062", "retirement@company.com", "401k Contribution Reminder", "Don't forget to review your 401k contributions for the new year.", "2024-02-05 13:00:00", 0, "thread_401k_042", None, None, None),
        ("email_063", "leadership@company.com", "Leadership Development Program", "Leadership development program available. Grow your leadership skills!", "2024-02-06 12:00:00", 0, "thread_leadership_042", None, None, None),
        ("email_064", "competitive@company.com", "Competitive Analysis Report", "Competitive analysis report is ready. Stay ahead of the competition!", "2024-02-07 11:00:00", 0, "thread_competitive_042", None, None, None),
        ("email_065", "audit@company.com", "Audit Preparation", "Audit preparation checklist available. Ensure compliance readiness!", "2024-02-08 13:00:00", 0, "thread_audit_prep_042", None, None, None),
        ("email_066", "crm@company.com", "Customer Relationship Management", "CRM training session scheduled. Improve your customer relationships!", "2024-02-09 14:00:00", 0, "thread_crm_042", None, None, None),
        ("email_067", "intellectual@company.com", "Intellectual Property", "Intellectual property training available. Protect your innovations!", "2024-02-10 11:00:00", 0, "thread_ip_042", None, None, None),
        ("email_068", "hackathon@company.com", "Hackathon Planning", "Annual hackathon planning begins. Share your ideas and register!", "2024-02-11 15:00:00", 0, "thread_hackathon_042", None, None, None),
        ("email_069", "innovation@company.com", "Innovation Lab Tour", "Innovation lab tour scheduled. See our latest prototypes and experiments.", "2024-02-12 10:00:00", 0, "thread_innovation_042", None, None, None),
        ("email_070", "sustainability@company.com", "Sustainability Initiative", "New sustainability initiative launched. Help us go green!", "2024-02-13 09:00:00", 0, "thread_sustainability_042", None, None, None),
        ("email_071", "diversity@company.com", "Diversity and Inclusion", "Diversity and inclusion workshop scheduled. All employees welcome.", "2024-02-13 14:00:00", 0, "thread_diversity_042", None, None, None),
        ("email_072", "mentorship@company.com", "Mentorship Program", "Mentorship program sign-up is open. Be a mentor or find a mentor.", "2024-02-14 08:00:00", 0, "thread_mentorship_042", None, None, None),
        ("email_073", "volunteer@company.com", "Volunteer Opportunities", "Volunteer opportunities available. Give back to the community!", "2024-02-14 12:00:00", 0, "thread_volunteer_042", None, None, None),
        ("email_074", "photography@company.com", "Company Photo Session", "Annual company photo session scheduled. Professional headshots available.", "2024-02-01 15:00:00", 0, "thread_photos_042", None, None, None),
        ("email_075", "birthday@company.com", "February Birthdays", "Happy birthday to our February team members! 🎂", "2024-02-01 12:00:00", 0, "thread_birthdays_042", None, None, None),
        ("email_076", "anniversary@company.com", "Work Anniversaries", "Congratulations to our work anniversary celebrants this month!", "2024-02-02 11:00:00", 0, "thread_anniversaries_042", None, None, None),
        ("email_077", "achievement@company.com", "Employee Achievements", "Celebrating our team's recent achievements and milestones!", "2024-02-03 13:00:00", 0, "thread_achievements_042", None, None, None),
        ("email_078", "recognition@company.com", "Employee Recognition", "Employee recognition program highlights. Nominate your colleagues!", "2024-02-04 12:00:00", 0, "thread_recognition_042", None, None, None),
        ("email_079", "feedback@company.com", "360 Feedback", "360-degree feedback process is open. Share your insights.", "2024-02-05 15:00:00", 0, "thread_360_042", None, None, None),
        ("email_080", "survey@company.com", "Employee Satisfaction Survey", "Annual employee satisfaction survey is live. Your voice matters!", "2024-02-06 10:00:00", 0, "thread_satisfaction_042", None, None, None),
    ]
    
    # Insert important emails
    for email in important_emails:
        cursor.execute("""
            INSERT OR IGNORE INTO emails (custom_id, sender, subject, body, received_date, is_read, thread_id, meeting_date, meeting_duration, attendees)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, email)
    
    # Insert noise emails
    for email in noise_emails:
        cursor.execute("""
            INSERT OR IGNORE INTO emails (custom_id, sender, subject, body, received_date, is_read, thread_id, meeting_date, meeting_duration, attendees)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, email)
    
    conn.commit()
    conn.close()
    print("✅ Email database created for Test Case 3")

def create_calendar_database():
    """Create and populate calendar database for Test Case 3"""
    conn = sqlite3.connect("data/databases/calendar.db")
    cursor = conn.cursor()
    
    # Create calendar table if it doesn't exist
    cursor.execute("""
        CREATE TABLE IF NOT EXISTS calendar_events (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            custom_id TEXT UNIQUE,
            title TEXT NOT NULL,
            description TEXT NOT NULL,
            start_date TEXT NOT NULL,
            end_date TEXT NOT NULL,
            location TEXT,
            attendees TEXT,
            is_all_day BOOLEAN DEFAULT 0,
            event_type TEXT DEFAULT 'meeting'
        )
    """)
    
    # Important calendar events (20% of total - 12 out of 60)
    important_events = [
        # Critical security and system events
        ("event_021", "Security Vulnerability Patch Deployment", "Critical security patch deployment for authentication system vulnerability. Immediate action required.", "2024-02-05 14:00:00", "2024-02-05 16:00:00", "Conference Room A", "security@company.com,cto@company.com,devops@company.com", 0, "critical"),
        ("event_025", "Q1 Strategic Planning Session", "Critical Q1 strategic planning session for key business decisions and roadmap alignment.", "2024-02-07 14:00:00", "2024-02-07 16:00:00", "Boardroom", "ceo@company.com,managers@company.com,architects@company.com", 0, "strategic"),
        ("event_027", "Q1 Budget Approval Meeting", "Approve Q1 budget for business operations and resource allocation.", "2024-02-09 10:00:00", "2024-02-09 11:00:00", "Conference Room B", "finance@company.com,ceo@company.com,managers@company.com", 0, "financial"),
        ("event_028", "Security Audit Preparation", "Prepare for annual security audit. Documentation review and system access verification.", "2024-02-11 10:00:00", "2024-02-11 13:00:00", "Security Office", "security@company.com,compliance@company.com,it@company.com", 0, "compliance"),
        
        # Customer and business critical events
        ("event_029", "Critical Customer Issue Resolution", "Resolve major customer system outage. Technical expertise required.", "2024-02-12 09:00:00", "2024-02-12 11:00:00", "War Room", "support@company.com,devops@company.com,cto@company.com", 0, "critical"),
        ("event_030", "Enterprise Deal Closing", "Major enterprise deal closing requires technical consultation and approval.", "2024-02-14 11:00:00", "2024-02-14 12:30:00", "Executive Conference Room", "sales@company.com,ceo@company.com,legal@company.com", 0, "business"),
        ("event_031", "Infrastructure Migration", "Critical infrastructure migration requires oversight and technical guidance.", "2024-02-13 20:00:00", "2024-02-14 02:00:00", "Data Center", "devops@company.com,operations@company.com,dev-team@company.com", 0, "technical"),
        ("event_032", "Product Launch Approval", "Final approval for major product launch. Go/no-go decision required.", "2024-02-13 15:00:00", "2024-02-13 16:00:00", "Product Office", "product@company.com,managers@company.com,qa@company.com", 0, "product"),
        
        # Q1 Planning and Performance
        ("event_033", "Q1 Product Roadmap Review", "Review and approve Q1 product roadmap. Strategic alignment session.", "2024-02-07 14:00:00", "2024-02-07 16:00:00", "Product Conference Room", "product@company.com,architects@company.com,managers@company.com", 0, "strategic"),
        ("event_034", "Annual Performance Review", "Annual performance review deadline. Submit self-assessment and manager review.", "2024-02-14 14:00:00", "2024-02-14 15:00:00", "HR Office", "hr@company.com,managers@company.com", 0, "hr"),
        ("event_035", "Database Performance Optimization", "Database performance optimization project review and approval.", "2024-02-08 15:00:00", "2024-02-08 16:30:00", "Tech Lab", "database@company.com,devops@company.com,architects@company.com", 0, "technical"),
        ("event_036", "Production System Maintenance", "Production system maintenance window. Downtime approval required.", "2024-02-06 02:00:00", "2024-02-06 06:00:00", "Operations Center", "operations@company.com,devops@company.com,cto@company.com", 0, "maintenance"),
    ]
    
    # Noise calendar events (80% of total - 48 out of 60)
    noise_events = [
        # Regular meetings and training
        ("event_037", "Weekly Team Standup", "Regular weekly team standup meeting.", "2024-02-01 09:00:00", "2024-02-01 09:30:00", "Team Room", "team@company.com", 0, "meeting"),
        ("event_038", "New Employee Onboarding", "Welcome new team members with onboarding session.", "2024-02-01 10:00:00", "2024-02-01 12:00:00", "Training Room", "hr@company.com,new-employees@company.com", 0, "training"),
        ("event_039", "Marketing Campaign Review", "Review current marketing campaigns and performance metrics.", "2024-02-01 14:00:00", "2024-02-01 15:00:00", "Marketing Office", "marketing@company.com", 0, "meeting"),
        ("event_040", "Customer Support Training", "Customer support team training session.", "2024-02-02 10:00:00", "2024-02-02 12:00:00", "Training Room", "support@company.com", 0, "training"),
        ("event_041", "Sales Team Meeting", "Weekly sales team meeting and pipeline review.", "2024-02-02 14:00:00", "2024-02-02 15:00:00", "Sales Office", "sales@company.com", 0, "meeting"),
        ("event_042", "IT Security Awareness", "IT security awareness training for all employees.", "2024-02-03 11:00:00", "2024-02-03 12:00:00", "Auditorium", "it@company.com,all@company.com", 0, "training"),
        ("event_043", "Project Status Update", "Weekly project status update meeting.", "2024-02-03 15:00:00", "2024-02-03 16:00:00", "Project Room", "project@company.com", 0, "meeting"),
        ("event_044", "Skills Development Workshop", "Professional skills development workshop.", "2024-02-04 09:00:00", "2024-02-04 12:00:00", "Training Room", "hr@company.com", 0, "training"),
        ("event_045", "Budget Planning Session", "Plan budget allocation for next quarter.", "2024-02-04 14:00:00", "2024-02-04 16:00:00", "Finance Office", "finance@company.com", 0, "meeting"),
        ("event_046", "Team Building Activity", "Monthly team building activity and games.", "2024-02-05 16:00:00", "2024-02-05 18:00:00", "Recreation Room", "hr@company.com", 0, "social"),
        
        # Valentine's Day themed events
        ("event_047", "Valentine's Day Office Decorations", "Decorate the office for Valentine's Day celebration.", "2024-02-09 12:00:00", "2024-02-09 13:00:00", "Main Office", "events@company.com", 0, "social"),
        ("event_048", "Valentine's Day Gift Exchange", "Valentine's Day gift exchange and celebration.", "2024-02-14 16:00:00", "2024-02-14 18:00:00", "Main Office", "events@company.com,all@company.com", 0, "social"),
        ("event_049", "Valentine's Day Lunch", "Special Valentine's Day lunch for all employees.", "2024-02-14 12:00:00", "2024-02-14 13:00:00", "Cafeteria", "catering@company.com,all@company.com", 0, "social"),
        
        # More regular meetings and events
        ("event_050", "Weekly All-Hands Meeting", "Weekly all-hands meeting with company updates.", "2024-02-06 10:00:00", "2024-02-06 11:00:00", "Auditorium", "all@company.com", 0, "meeting"),
        ("event_051", "Code Review Session", "Weekly code review session for development team.", "2024-02-06 15:00:00", "2024-02-06 16:00:00", "Dev Room", "dev-team@company.com", 0, "meeting"),
        ("event_052", "Customer Feedback Review", "Review customer feedback and improvement suggestions.", "2024-02-07 10:00:00", "2024-02-07 11:00:00", "Product Office", "product@company.com", 0, "meeting"),
        ("event_053", "Vendor Meeting", "Monthly vendor meeting and contract review.", "2024-02-07 15:00:00", "2024-02-07 16:00:00", "Procurement Office", "procurement@company.com", 0, "meeting"),
        ("event_054", "Quality Assurance Review", "Quality assurance review and testing updates.", "2024-02-08 10:00:00", "2024-02-08 11:00:00", "QA Office", "qa@company.com", 0, "meeting"),
        ("event_055", "Operations Review", "Weekly operations review and performance metrics.", "2024-02-08 14:00:00", "2024-02-08 15:00:00", "Operations Center", "operations@company.com", 0, "meeting"),
        ("event_056", "Research and Development", "R&D team meeting and innovation discussion.", "2024-02-09 10:00:00", "2024-02-09 12:00:00", "R&D Lab", "research@company.com", 0, "meeting"),
        ("event_057", "Compliance Training", "Monthly compliance training session.", "2024-02-09 14:00:00", "2024-02-09 15:00:00", "Training Room", "compliance@company.com", 0, "training"),
        ("event_058", "Customer Success Meeting", "Customer success team meeting and case review.", "2024-02-10 10:00:00", "2024-02-10 11:00:00", "Customer Success Office", "customer-success@company.com", 0, "meeting"),
        ("event_059", "Marketing Strategy Session", "Marketing strategy session and campaign planning.", "2024-02-10 14:00:00", "2024-02-10 16:00:00", "Marketing Office", "marketing@company.com", 0, "meeting"),
        ("event_060", "Technology Review", "Technology review and architecture discussion.", "2024-02-11 15:00:00", "2024-02-11 16:00:00", "Tech Lab", "architects@company.com", 0, "meeting"),
        ("event_061", "Employee Recognition", "Monthly employee recognition and awards ceremony.", "2024-02-12 11:00:00", "2024-02-12 12:00:00", "Auditorium", "hr@company.com,all@company.com", 0, "celebration"),
        ("event_062", "Innovation Lab Tour", "Innovation lab tour and prototype demonstration.", "2024-02-12 15:00:00", "2024-02-12 16:00:00", "Innovation Lab", "innovation@company.com", 0, "tour"),
        ("event_063", "Sustainability Workshop", "Sustainability workshop and green initiative discussion.", "2024-02-13 10:00:00", "2024-02-13 12:00:00", "Training Room", "sustainability@company.com", 0, "workshop"),
        ("event_064", "Diversity and Inclusion", "Diversity and inclusion workshop and discussion.", "2024-02-13 14:00:00", "2024-02-13 16:00:00", "Training Room", "diversity@company.com", 0, "workshop"),
        ("event_065", "Mentorship Program", "Mentorship program kickoff and pairing session.", "2024-02-14 09:00:00", "2024-02-14 10:00:00", "Training Room", "hr@company.com", 0, "training"),
        ("event_066", "Volunteer Opportunities", "Volunteer opportunities presentation and sign-up.", "2024-02-14 13:00:00", "2024-02-14 14:00:00", "Community Room", "volunteer@company.com", 0, "presentation"),
    ]
    
    # Insert important events
    for event in important_events:
        cursor.execute("""
            INSERT OR IGNORE INTO calendar_events (custom_id, title, description, start_date, end_date, location, attendees, is_all_day, event_type)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, event)
    
    # Insert noise events
    for event in noise_events:
        cursor.execute("""
            INSERT OR IGNORE INTO calendar_events (custom_id, title, description, start_date, end_date, location, attendees, is_all_day, event_type)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, event)
    
    conn.commit()
    conn.close()
    print("✅ Calendar database created for Test Case 3")

def create_slack_database():
    """Create and populate Slack database for Test Case 3"""
    conn = sqlite3.connect("data/databases/slack.db")
    cursor = conn.cursor()
    
    # Create slack_messages table if it doesn't exist
    cursor.execute("""
        CREATE TABLE IF NOT EXISTS slack_messages (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            custom_id TEXT UNIQUE,
            channel TEXT NOT NULL,
            sender TEXT NOT NULL,
            message TEXT NOT NULL,
            timestamp TEXT NOT NULL,
            thread_ts TEXT,
            is_urgent BOOLEAN DEFAULT 0,
            mentions TEXT
        )
    """)
    
    # Important Slack messages (20% of total - 20 out of 100)
    important_messages = [
        # Critical security and system messages
        ("slack_021", "#security", "security-bot", "URGENT: Critical security vulnerability in auth system. Patch deployment required by Feb 5th.", "2024-02-05 09:00:00", None, 1, "cto@company.com,devops@company.com"),
        ("slack_030", "#incidents", "support-lead", "Critical customer issue - system outage. Your technical expertise needed for resolution.", "2024-02-12 08:00:00", None, 1, "devops@company.com,cto@company.com"),
        ("slack_043", "#enterprise-deals", "sales-director", "Enterprise deal closing Feb 14th. Technical consultation and approval required.", "2024-02-14 10:00:00", None, 1, "ceo@company.com,legal@company.com"),
        ("slack_110", "#infrastructure", "devops-lead", "Infrastructure migration Feb 13th. Your oversight and technical guidance required.", "2024-02-13 08:30:00", None, 1, "operations@company.com,dev-team@company.com"),
        
        # Q1 Planning and Budget
        ("slack_031", "#q1-planning", "product-manager", "Q1 strategic planning session Feb 7th. Critical business decisions to be made.", "2024-02-06 15:00:00", None, 1, "ceo@company.com,managers@company.com"),
        ("slack_032", "#budget", "finance-director", "Q1 budget approval required Feb 9th. Critical for business operations.", "2024-02-08 14:00:00", None, 1, "ceo@company.com,managers@company.com"),
        ("slack_033", "#roadmap", "product-lead", "Q1 product roadmap review Feb 7th. Strategic alignment session.", "2024-02-06 16:00:00", None, 1, "architects@company.com,managers@company.com"),
        
        # Security and Compliance
        ("slack_034", "#compliance", "compliance-officer", "Security audit preparation Feb 11th. Documentation review required.", "2024-02-09 11:00:00", None, 1, "security@company.com,it@company.com"),
        ("slack_035", "#legal", "legal-counsel", "Major client contract renewal deadline Feb 10th. Legal review required.", "2024-02-08 10:30:00", None, 1, "ceo@company.com,sales@company.com"),
        ("slack_036", "#performance", "hr-director", "Annual performance review deadline Feb 14th. Submit self-assessment.", "2024-02-13 10:00:00", None, 1, "managers@company.com"),
        
        # System Maintenance
        ("slack_037", "#operations", "ops-manager", "Production system maintenance Feb 6th. Downtime approval required.", "2024-02-05 16:00:00", None, 1, "devops@company.com,cto@company.com"),
        ("slack_038", "#database", "db-admin", "Database performance optimization Feb 8th. Technical review required.", "2024-02-07 11:00:00", None, 1, "devops@company.com,architects@company.com"),
        ("slack_039", "#releases", "qa-lead", "Release testing Feb 14th. Approval required before production deployment.", "2024-02-14 09:00:00", None, 1, "dev-team@company.com,product@company.com"),
        ("slack_040", "#security-incidents", "security-lead", "Security incident response Feb 14th. Immediate technical expertise required.", "2024-02-14 12:00:00", None, 1, "cto@company.com,operations@company.com"),
        
        # Valentine's Day Context
        ("slack_041", "#events", "events-coordinator", "Valentine's Day team building event Feb 14th. Your participation encouraged.", "2024-02-12 14:00:00", None, 0, "all@company.com"),
        ("slack_042", "#marketing", "marketing-manager", "Valentine's Day campaign launch Feb 13th. Approval required.", "2024-02-12 13:00:00", None, 1, "product@company.com,managers@company.com"),
        
        # Additional Critical Items
        ("slack_043", "#customer-success", "customer-success-manager", "Major customer escalation. Immediate attention required.", "2024-02-11 15:00:00", None, 1, "support@company.com,cto@company.com"),
        ("slack_044", "#product-launch", "product-manager", "Product launch approval Feb 13th. Go/no-go decision required.", "2024-02-13 15:00:00", None, 1, "managers@company.com,qa@company.com"),
        ("slack_045", "#quarterly-review", "compliance-officer", "Quarterly compliance review due Feb 15th. Documentation update required.", "2024-02-12 09:00:00", None, 1, "legal@company.com,all@company.com"),
        ("slack_046", "#innovation", "innovation-lead", "Innovation lab prototype review. Technical evaluation required.", "2024-02-12 10:00:00", None, 1, "architects@company.com,product@company.com"),
    ]
    
    # Noise Slack messages (80% of total - 80 out of 100)
    noise_messages = [
        # Regular team communication
        ("slack_047", "#general", "team-member-1", "Good morning team! Ready for another productive day.", "2024-02-01 08:00:00", None, 0, None),
        ("slack_048", "#random", "team-member-2", "Anyone know where the coffee machine is?", "2024-02-01 08:30:00", None, 0, None),
        ("slack_049", "#announcements", "hr-bot", "Welcome our new team members! Please introduce yourselves.", "2024-02-01 09:00:00", None, 0, "all@company.com"),
        ("slack_050", "#tech-talk", "dev-1", "Just learned about this new framework. Anyone tried it?", "2024-02-01 10:00:00", None, 0, None),
        ("slack_051", "#watercooler", "team-member-3", "Did anyone see the latest episode of that show?", "2024-02-01 12:00:00", None, 0, None),
        ("slack_052", "#lunch", "team-member-4", "Lunch at the new restaurant today. Anyone want to join?", "2024-02-01 12:30:00", None, 0, None),
        ("slack_053", "#feedback", "product-manager", "Thanks for the great feedback on the new feature!", "2024-02-01 15:00:00", None, 0, None),
        ("slack_054", "#celebrations", "hr-bot", "Happy birthday to our February team members! 🎂", "2024-02-01 16:00:00", None, 0, "all@company.com"),
        ("slack_055", "#office-updates", "admin-bot", "Office supplies have been restocked. Thanks for your patience!", "2024-02-02 09:00:00", None, 0, None),
        ("slack_056", "#wellness", "wellness-coordinator", "Don't forget to take breaks and stay hydrated!", "2024-02-02 10:00:00", None, 0, "all@company.com"),
        
        # More noise messages to reach 80 total
        ("slack_057", "#general", "team-member-5", "Great job on the project everyone!", "2024-02-02 11:00:00", None, 0, None),
        ("slack_058", "#random", "team-member-6", "Weather is looking nice today!", "2024-02-02 12:00:00", None, 0, None),
        ("slack_059", "#tech-talk", "dev-2", "Interesting article about AI trends.", "2024-02-02 14:00:00", None, 0, None),
        ("slack_060", "#watercooler", "team-member-7", "Anyone have recommendations for good books?", "2024-02-02 15:00:00", None, 0, None),
        ("slack_061", "#lunch", "team-member-8", "Pizza Friday! What's everyone's favorite topping?", "2024-02-02 16:00:00", None, 0, None),
        ("slack_062", "#feedback", "customer-success", "Customer feedback has been great this week!", "2024-02-03 09:00:00", None, 0, None),
        ("slack_063", "#celebrations", "team-member-9", "Congratulations on the work anniversary!", "2024-02-03 10:00:00", None, 0, None),
        ("slack_064", "#office-updates", "facilities", "New plants have been added to the office!", "2024-02-03 11:00:00", None, 0, None),
        ("slack_065", "#wellness", "fitness-coordinator", "Yoga session in the conference room at 5 PM!", "2024-02-03 16:00:00", None, 0, "all@company.com"),
        ("slack_066", "#general", "team-member-10", "Have a great weekend everyone!", "2024-02-03 17:00:00", None, 0, None),
        
        # Continue with more noise messages...
        ("slack_067", "#random", "team-member-11", "Monday motivation! Let's make it a great week!", "2024-02-05 08:00:00", None, 0, None),
        ("slack_068", "#tech-talk", "dev-3", "New coding challenge posted. Who's up for it?", "2024-02-05 10:00:00", None, 0, None),
        ("slack_069", "#watercooler", "team-member-12", "Anyone tried the new coffee blend?", "2024-02-05 11:00:00", None, 0, None),
        ("slack_070", "#lunch", "team-member-13", "Food truck is here! Great tacos today.", "2024-02-05 12:00:00", None, 0, None),
        ("slack_071", "#feedback", "product-manager", "User testing results are in. Very positive!", "2024-02-05 14:00:00", None, 0, None),
        ("slack_072", "#celebrations", "hr-bot", "Employee of the month announcement coming soon!", "2024-02-05 15:00:00", None, 0, "all@company.com"),
        ("slack_073", "#office-updates", "admin-bot", "Parking permits expire at month-end. Renew online!", "2024-02-05 16:00:00", None, 0, None),
        ("slack_074", "#wellness", "wellness-coordinator", "Mental health awareness session this Friday.", "2024-02-05 17:00:00", None, 0, "all@company.com"),
        ("slack_075", "#general", "team-member-14", "Thanks for all the hard work this week!", "2024-02-05 18:00:00", None, 0, None),
        ("slack_076", "#random", "team-member-15", "Weekend plans anyone?", "2024-02-05 18:30:00", None, 0, None),
        
        # Continue with remaining noise messages to reach 80 total
        ("slack_077", "#tech-talk", "dev-4", "Code review session went well!", "2024-02-06 09:00:00", None, 0, None),
        ("slack_078", "#watercooler", "team-member-16", "Anyone watching the game tonight?", "2024-02-06 12:00:00", None, 0, None),
        ("slack_079", "#lunch", "team-member-17", "Healthy lunch options in the cafeteria today!", "2024-02-06 12:30:00", None, 0, None),
        ("slack_080", "#feedback", "customer-success", "Customer satisfaction scores are up!", "2024-02-06 15:00:00", None, 0, None),
        ("slack_081", "#celebrations", "team-member-18", "Happy work anniversary!", "2024-02-06 16:00:00", None, 0, None),
        ("slack_082", "#office-updates", "facilities", "New ergonomic chairs have arrived!", "2024-02-06 17:00:00", None, 0, None),
        ("slack_083", "#wellness", "fitness-coordinator", "Walking group meeting at 5:30 PM!", "2024-02-06 17:30:00", None, 0, "all@company.com"),
        ("slack_084", "#general", "team-member-19", "Great collaboration on the project!", "2024-02-07 08:00:00", None, 0, None),
        ("slack_085", "#random", "team-member-20", "Coffee break anyone?", "2024-02-07 10:00:00", None, 0, None),
        ("slack_086", "#tech-talk", "dev-5", "Interesting discussion about microservices!", "2024-02-07 11:00:00", None, 0, None),
        ("slack_087", "#watercooler", "team-member-21", "Anyone have good podcast recommendations?", "2024-02-07 12:00:00", None, 0, None),
        ("slack_088", "#lunch", "team-member-22", "Team lunch at the new restaurant!", "2024-02-07 12:30:00", None, 0, None),
        ("slack_089", "#feedback", "product-manager", "Feature request summary is ready!", "2024-02-07 15:00:00", None, 0, None),
        ("slack_090", "#celebrations", "hr-bot", "Team building event sign-up is open!", "2024-02-07 16:00:00", None, 0, "all@company.com"),
        ("slack_091", "#office-updates", "admin-bot", "New printer setup in the copy room!", "2024-02-07 17:00:00", None, 0, None),
        ("slack_092", "#wellness", "wellness-coordinator", "Stress management workshop this week!", "2024-02-07 17:30:00", None, 0, "all@company.com"),
        ("slack_093", "#general", "team-member-23", "Thanks for the great teamwork!", "2024-02-08 08:00:00", None, 0, None),
        ("slack_094", "#random", "team-member-24", "TGIF! Have a great weekend!", "2024-02-08 17:00:00", None, 0, None),
        ("slack_095", "#tech-talk", "dev-6", "New development tools training next week!", "2024-02-09 09:00:00", None, 0, None),
        ("slack_096", "#watercooler", "team-member-25", "Anyone tried the new restaurant downtown?", "2024-02-09 12:00:00", None, 0, None),
        ("slack_097", "#lunch", "team-member-26", "Potluck lunch next Friday!", "2024-02-09 12:30:00", None, 0, None),
        ("slack_098", "#feedback", "customer-success", "Customer testimonials are amazing!", "2024-02-09 15:00:00", None, 0, None),
        ("slack_099", "#celebrations", "team-member-27", "Congratulations on the promotion!", "2024-02-09 16:00:00", None, 0, None),
        ("slack_100", "#office-updates", "facilities", "Office renovation starting next month!", "2024-02-09 17:00:00", None, 0, None),
    ]
    
    # Insert important messages
    for message in important_messages:
        cursor.execute("""
            INSERT OR IGNORE INTO slack_messages (custom_id, channel, sender, message, timestamp, thread_ts, is_urgent, mentions)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        """, message)
    
    # Insert noise messages
    for message in noise_messages:
        cursor.execute("""
            INSERT OR IGNORE INTO slack_messages (custom_id, channel, sender, message, timestamp, thread_ts, is_urgent, mentions)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        """, message)
    
    conn.commit()
    conn.close()
    print("✅ Slack database created for Test Case 3")

def create_kanban_database():
    """Create and populate Kanban database for Test Case 3"""
    conn = sqlite3.connect("data/databases/kanban.db")
    cursor = conn.cursor()
    
    # Create project_progress table if it doesn't exist
    cursor.execute("""
        CREATE TABLE IF NOT EXISTS project_progress (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            custom_id TEXT UNIQUE,
            project_name TEXT NOT NULL,
            milestone TEXT NOT NULL,
            progress_percentage INTEGER NOT NULL,
            status TEXT NOT NULL,
            due_date TEXT NOT NULL,
            updated_date TEXT NOT NULL
        )
    """)
    
    # Create task_updates table if it doesn't exist
    cursor.execute("""
        CREATE TABLE IF NOT EXISTS task_updates (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            custom_id TEXT UNIQUE,
            task_id TEXT NOT NULL,
            update_type TEXT NOT NULL,
            description TEXT NOT NULL,
            updated_by TEXT NOT NULL,
            updated_date TEXT NOT NULL,
            new_status TEXT
        )
    """)
    
    # Important project progress (20% of total - 12 out of 60)
    important_progress = [
        # Critical security and system projects
        ("task_021", "Security Patch Deployment", "Critical Security Fix", 85, "in_progress", "2024-02-05", "2024-02-05 09:00:00"),
        ("task_022", "Infrastructure Migration", "System Migration", 70, "in_progress", "2024-02-13", "2024-02-13 08:30:00"),
        ("task_023", "Customer System Recovery", "Outage Resolution", 60, "in_progress", "2024-02-12", "2024-02-12 08:00:00"),
        ("task_024", "Enterprise Deal Support", "Technical Consultation", 90, "in_progress", "2024-02-14", "2024-02-14 10:00:00"),
        
        # Q1 Planning and Budget
        ("task_025", "Q1 Strategic Planning", "Roadmap Alignment", 80, "in_progress", "2024-02-07", "2024-02-07 14:00:00"),
        ("task_026", "Q1 Budget Approval", "Financial Planning", 95, "in_progress", "2024-02-09", "2024-02-09 10:00:00"),
        ("task_027", "Q1 Product Roadmap", "Feature Planning", 75, "in_progress", "2024-02-07", "2024-02-07 14:00:00"),
        
        # Security and Compliance
        ("task_028", "Security Audit Preparation", "Compliance Review", 65, "in_progress", "2024-02-11", "2024-02-11 10:00:00"),
        ("task_029", "Contract Renewal", "Legal Review", 85, "in_progress", "2024-02-10", "2024-02-10 09:00:00"),
        ("task_030", "Performance Review", "HR Process", 70, "in_progress", "2024-02-14", "2024-02-14 14:00:00"),
        
        # System Maintenance
        ("task_031", "Production Maintenance", "System Downtime", 90, "in_progress", "2024-02-06", "2024-02-06 02:00:00"),
        ("task_032", "Database Optimization", "Performance Tuning", 60, "in_progress", "2024-02-08", "2024-02-08 15:00:00"),
    ]
    
    # Important task updates (20% of total - 12 out of 60)
    important_updates = [
        # Critical updates
        ("update_021", "task_021", "status_change", "Security patch deployment in progress. Critical vulnerability being addressed.", "security-lead", "2024-02-05 09:00:00", "in_progress"),
        ("update_022", "task_022", "progress_update", "Infrastructure migration 70% complete. Final phase starting.", "devops-lead", "2024-02-13 08:30:00", "in_progress"),
        ("update_023", "task_023", "status_change", "Customer system outage investigation in progress. Root cause identified.", "support-lead", "2024-02-12 08:00:00", "in_progress"),
        ("update_024", "task_024", "progress_update", "Enterprise deal technical consultation 90% complete. Final review pending.", "sales-director", "2024-02-14 10:00:00", "in_progress"),
        
        # Q1 Planning updates
        ("update_025", "task_025", "milestone_update", "Q1 strategic planning session completed. Roadmap aligned with business goals.", "product-manager", "2024-02-07 14:00:00", "in_progress"),
        ("update_026", "task_026", "status_change", "Q1 budget approval process 95% complete. Final sign-off pending.", "finance-director", "2024-02-09 10:00:00", "in_progress"),
        ("update_027", "task_027", "progress_update", "Q1 product roadmap review completed. Feature priorities established.", "product-lead", "2024-02-07 14:00:00", "in_progress"),
        
        # Security and compliance updates
        ("update_028", "task_028", "status_change", "Security audit preparation 65% complete. Documentation review in progress.", "compliance-officer", "2024-02-11 10:00:00", "in_progress"),
        ("update_029", "task_029", "progress_update", "Contract renewal legal review 85% complete. Final terms being negotiated.", "legal-counsel", "2024-02-10 09:00:00", "in_progress"),
        ("update_030", "task_030", "milestone_update", "Performance review process 70% complete. Self-assessments being collected.", "hr-director", "2024-02-14 14:00:00", "in_progress"),
        
        # System maintenance updates
        ("update_031", "task_031", "status_change", "Production maintenance window 90% complete. System restart successful.", "ops-manager", "2024-02-06 02:00:00", "in_progress"),
        ("update_032", "task_032", "progress_update", "Database optimization 60% complete. Performance improvements implemented.", "db-admin", "2024-02-08 15:00:00", "in_progress"),
    ]
    
    # Insert important progress
    for progress in important_progress:
        cursor.execute("""
            INSERT OR IGNORE INTO project_progress (custom_id, project_name, milestone, progress_percentage, status, due_date, updated_date)
            VALUES (?, ?, ?, ?, ?, ?, ?)
        """, progress)
    
    # Insert important updates
    for update in important_updates:
        cursor.execute("""
            INSERT OR IGNORE INTO task_updates (custom_id, task_id, update_type, description, updated_by, updated_date, new_status)
            VALUES (?, ?, ?, ?, ?, ?, ?)
        """, update)
    
    conn.commit()
    conn.close()
    print("✅ Kanban database created for Test Case 3")

def create_github_database():
    """Create and populate GitHub database for Test Case 3"""
    conn = sqlite3.connect("data/databases/github.db")
    cursor = conn.cursor()
    
    # Create commits table if it doesn't exist
    cursor.execute("""
        CREATE TABLE IF NOT EXISTS commits (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            repository TEXT NOT NULL,
            author TEXT NOT NULL,
            message TEXT NOT NULL,
            commit_hash TEXT NOT NULL,
            branch TEXT NOT NULL,
            files_changed INTEGER DEFAULT 0,
            lines_added INTEGER DEFAULT 0,
            lines_deleted INTEGER DEFAULT 0,
            commit_date TEXT NOT NULL,
            custom_id TEXT UNIQUE
        )
    """)
    
    # Important commits (20% of total - 8 out of 40)
    important_commits = [
        # Critical security and system commits
        ("commit_021", "security-patches", "security-dev", "Fix critical authentication vulnerability", "abc123def", "main", 15, 200, 50, "2024-02-05 10:00:00"),
        ("commit_022", "infrastructure", "devops-dev", "Implement infrastructure migration scripts", "def456ghi", "main", 25, 300, 100, "2024-02-13 09:00:00"),
        ("commit_023", "customer-support", "support-dev", "Fix customer system outage issue", "ghi789jkl", "hotfix", 8, 120, 30, "2024-02-12 08:30:00"),
        ("commit_024", "enterprise-features", "backend-dev", "Add enterprise deal support features", "jkl012mno", "feature/enterprise", 20, 250, 80, "2024-02-14 11:00:00"),
        
        # Q1 Planning and features
        ("commit_025", "product-roadmap", "product-dev", "Implement Q1 roadmap features", "mno345pqr", "feature/q1-roadmap", 18, 180, 60, "2024-02-07 15:00:00"),
        ("commit_026", "budget-system", "finance-dev", "Add Q1 budget approval workflow", "pqr678stu", "feature/budget", 12, 150, 40, "2024-02-09 10:30:00"),
        ("commit_027", "security-audit", "security-dev", "Prepare security audit documentation", "stu901vwx", "feature/audit", 10, 100, 20, "2024-02-11 10:00:00"),
        ("commit_028", "performance-optimization", "backend-dev", "Optimize database performance", "vwx234yza", "feature/performance", 22, 280, 90, "2024-02-08 16:00:00"),
    ]
    
    # Insert important commits
    for commit in important_commits:
        cursor.execute("""
            INSERT OR IGNORE INTO commits (custom_id, repository, author, message, commit_hash, branch, files_changed, lines_added, lines_deleted, commit_date)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, commit)
    
    conn.commit()
    conn.close()
    print("✅ GitHub database created for Test Case 3")

def main():
    """Create all databases for Test Case 3"""
    print("🎯 Creating seed data for Test Case 3: 14-day OOO (Feb 1-14, 2024)")
    print("📅 Valentine's Day context with Q1 planning, security issues, and system maintenance")
    
    create_email_database()
    create_calendar_database()
    create_slack_database()
    create_kanban_database()
    create_github_database()
    
    print("✅ All databases created successfully for Test Case 3")

if __name__ == "__main__":
    main()
